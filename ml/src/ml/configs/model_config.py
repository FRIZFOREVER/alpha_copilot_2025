...
class ModelSettings(BaseModel):
    ...
    keep_alive: Optional[Union[int, str]] = Field(
        ...
        description=(
            "How long till ollama unloads a model from GPU"
        ),
    )
    ...
    num_ctx: int = Field(
        default=8192,
        ge=0,
        description="Sets maximum context length for response"
    )

    num_predict: int = Field(
        default=-1, # means unlimited possible output tokens
        description="Sets maximum amount of output tokens generated by model"
    )
