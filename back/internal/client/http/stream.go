...
// StreamClient клиент для работы с потоковыми сообщениями.
...
// NewStreamClient создает новый клиент.
func NewStreamClient(method, host, path string, historyLen int, logger *logrus.Logger) *StreamClient {
	return &StreamClient{
		method: method,
		host:   host,
		path:   path,
...
func (c *StreamClient) StreamRequestToModel(payload PayloadStream) (<-chan *StreamMessage, string, error) {
	var tag string
	...
	// Создаем HTTP запрос
	req, err := http.NewRequest(c.method, c.host+c.path, strings.NewReader(string(jsonData)))
	if err != nil {
...
func (c *StreamClient) processSSEStream(resp *http.Response, messageChan chan<- *StreamMessage) {
	defer func() {
		if err := resp.Body.Close(); err != nil {
			c.logger.Error("Ошибка при закрытии тела запроса: ", err)
		}
...
func (c *StreamClient) StreamRequestToModelWithTimeout(payload PayloadStream, timeout time.Duration) (<-chan *StreamMessage, string, error) {
	messageChan := make(chan *StreamMessage)
