...
func Test_History(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMocks: func(mr *MockHistoryRepository) {
				...
				mr.On("GetHistory", 1, "00000000-0000-0000-0000-000000000000", mock.Anything, mock.Anything).Return([]database.Message{
					// Элемент 1
					{
						...
						FileURL:      "https://example.com/files/go_benefits.pdf",
						Rating:       &intPtr,
					},
					...
					// Элемент 2
					{
						...
						VoiceURL:     "",
						FileURL:      "https://example.com/files/gc_explanation.pdf",
						Rating:       &intPtr,
					},
			...
			expectedStatus:   fiber.StatusOK,
			expectedResponse: `[{"question_id":1001,"answer_id":2001,"question":"Какие основные преимущества использования Go?","tag":"programming","answer":"Go предлагает простой синтаксис, эффективную сборку мусора, встроенную поддержку конкурентности через горутины и каналы, а также отличную производительность.","question_time":"2024-01-15T10:30:00Z","answer_time":"2024-01-15T10:35:00Z","voice_url":"https://example.com/voices/answer2001.mp3","file_url":"https://example.com/files/go_benefits.pdf","rating":5},{"question_id":1002,"answer_id":2002,"question":"Как работает сборка мусора в Go?","tag":"programming","answer":"В Go используется параллельная трехцветная маркировка и очистка с минимальными паузами, что позволяет эффективно управлять памятью без значительного влияния на производительность.","question_time":"2024-01-16T14:20:00Z","answer_time":"2024-01-16T14:25:00Z","voice_url":"","file_url":"https://example.com/files/gc_explanation.pdf","rating":5},{"question_id":1003,"answer_id":2003,"question":"Что такое горутины и чем они отличаются от потоков?","tag":null,"answer":"Горутины - это легковесные потоки, управляемые средой выполнения Go. Они потребляют меньше памяти чем системные потоки и создаются/уничтожаются быстрее. Горутины планируются средой выполнения Go, а не операционной системой.","question_time":"2024-01-17T09:15:00Z","answer_time":"2024-01-17T09:18:00Z","voice_url":"https://example.com/voices/goroutines.mp3","file_url":"","rating":null}]`,
		},
	}
