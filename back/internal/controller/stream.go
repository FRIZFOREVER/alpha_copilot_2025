...
func NewStream(service *service.Stream, logger *logrus.Logger) *Stream {
	return &Stream{
		service: service,
		logger:  logger,
	}
		...
		for message := range messageChan {
			if message != nil {
				...
				if message.Done {
					...
					finalChunk := dto.StreamChunckOut{
						Content: "",
						Time:    time.Now().UTC(),
						Done:    true,
					}
